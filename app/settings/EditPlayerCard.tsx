    ...
    >
      <div className="relative">
        <input
          ...
          className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-small focus:border-indigo focus:outline-none"
        />
        {dropdownOpen && (
            ...
            {results.map((p) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-small-dense text-ink-dim">{p.serieATeam}</span>
                </span>
              </button>
            ))}
            {results.length === 0 && (
        ...
        <div className="rounded-lg border border-border p-3">
          <div className="mb-3 flex items-center gap-2.5">
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="block text-small-dense text-ink-dim">{selected.serieATeam}</span>
            </span>
            <button
              onClick={() => setSelected(null)}
              className="flex-shrink-0 text-small-dense font-semibold text-ink-dim hover:text-coral"
            >
              Cambia
            </button>
          </div>
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-1.5 text-label uppercase text-ink-faint">
              Ruolo
              <select
                value={selected.role}
                onChange={(e) => changeRole(e.target.value as Role)}
                className="rounded-[7px] border border-border bg-surface px-1.5 py-1 text-small-dense font-semibold normal-case tracking-normal text-ink"
              >
                {ROLE_ORDER.map((r) => (
