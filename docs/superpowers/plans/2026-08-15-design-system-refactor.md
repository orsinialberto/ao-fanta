...
- **No arbitrary values in application code.** Every size, radius, colour and space comes from a token utility. `text-[13.5px]`, `p-[18px]`, `rounded-[14px]` and friends are defects, not shortcuts. The only exceptions are a one-off `w-[220px]`/`w-[296px]` for the two fixed layout columns named below, and `h-[2px]`/`h-[3px]` on the thin accent progress bars (sidebar auction status, team credit spend), where the spacing scale's smallest step (4px) is too thick for the intended hairline.
